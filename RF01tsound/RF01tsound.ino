...
boolean u0sync = 0, u1sync = 0, u2sync = 0, u3sync = 0;
String ID = "rf00";
void setup() {
  ...
  //delay(2000);
  display.clearDisplay();
  int extract = EEPROM.read(2); //por ser id2
  if (extract == 1) {
    u2sync = 1;
  }
  else {
  ...
  extract = EEPROM.read(1); //por ser id1
  if (extract == 1) {
    u1sync = 1;
  }
  else {
  ...
  extract = EEPROM.read(0); //por ser id1
  if (extract == 1) {
    u0sync = 1;
  }
  else {
  ...
  extract = EEPROM.read(3); //por ser id1
  if (extract == 1) {
    u3sync = 1;
  }
  else {
...
long previousMillis0 = 0; // will store last time of the cycle end
...
boolean alert0,alert1,alert2,alert3,remote1 = 0, remote2 = 0,borrar,motorstat,emparejar,stringcomplete = 0;
byte value;
...
void loop() {
  ...
  timmer1();
  if(u0sync|| u1sync|| u2sync|| u3sync){ timmer2();}
  //if (u2sync) { timmer3();}
  if (alert1 || alert2 || alert0 || alert3) { Alarma();
        motorvib(200);
      }

  if(!digitalRead(Button)){
    if (alert0 || alert1 || alert2|| alert3) {
      alert0 = 0;
      alert1 = 0;
      alert2 = 0;
      alert3 = 0;
...
void timmer2() {
  if (u0sync) {
    ...
    if (currentMillis0 - previousMillis0 >= Watsdog) { //Uptade every X miliseconds, this will be equal to reading frecuency (Hz).

      UsbDebug.println("Tiempo vencido UNIDAD 0");
      alert0 = 1;
  ...
  if (u1sync) {
    ...
    if (currentMillis1 - previousMillis1 >= Watsdog) { //Uptade every X miliseconds, this will be equal to reading frecuency (Hz).
  ...
  if (u2sync) {
    ...
    if (currentMillis2 - previousMillis2 >= Watsdog) { //Uptade every X miliseconds, this will be equal to reading frecuency (Hz).
  ...
  if (u3sync) {
    ...
    if (currentMillis3 - previousMillis3 >= Watsdog) { //Uptade every X miliseconds, this will be equal to reading frecuency (Hz).

      UsbDebug.println("Tiempo vencido UNIDAD 3");
      alert3 = 1;
...
void pantalla() {
  ...
  display.setTextColor(WHITE);
  display.setCursor(0, 0);
  if (u1sync && !alert1) { display.println("001| OK"); }
  else if(u1sync && alert1) {
    display.println("001|ERROR");
  }
  if (u2sync && !alert2) { display.println("002| OK"); }
  else if(u2sync && alert2){
    display.println("002|ERROR");
  }
  if (u3sync && !alert3) { display.println("003| OK"); }
  else if (u3sync && alert3) {
    display.println("003|ERROR");
  }
  if (u0sync && !alert0) { display.println("000| OK"); }
  else if (u0sync && alert0) {
    display.println("000|ERROR");
  }
  display.println(" ID|ESTADO");
  display.display();
  display.clearDisplay();
